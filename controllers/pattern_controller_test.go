...
var _ = Describe("pattern controller", func() {
	...
	var _ = Context("reconciliation", func() {
		...
		It("adding a pattern with origin, target and interval >-1", func() {
			...
			mockGitOps.EXPECT().CloneAndCheckout("https://target.url", "HEAD", "/tmp/vp/https___target.url", false, nil).Return(nil)
		...
		It("validates changes to the poll interval in the manifest", func() {
			mockGitOps.EXPECT().CloneAndCheckout("https://target.url", "HEAD", "/tmp/vp/https___target.url", false, nil).Return(nil).AnyTimes()
